...
# 加載數據
data_folder1 = datasets.ImageFolder(
    root='TOPIC/ProjectB/B_traing1', transform=train_transform)
data_folder2 = datasets.ImageFolder(
    root='TOPIC/ProjectB/B_traing2', transform=train_transform)
data_full = ConcatDataset([data_folder1, data_folder2])
...
# 主執行函數
labels = np.array([label for _, label in data_full.samples])
...
def run_training(args, device):
    ...
    for fold, (train_idx, val_idx) in enumerate(sss.split(np.zeros(len(labels)), labels)):
        print(f'Fold {fold + 1}')
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--model', type=str, default='EfficientNetB0', required=True, help='Model name')
